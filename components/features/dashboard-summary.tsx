...
function calculateTotals(transactions: Transaction[]) {
  ...
  transactions.forEach((t) => {
    const amount = getTransactionDisplayEurAmount(t);
    if (
      t.hide_from_totals ||
      t.is_money_transfer ||
      amount === null ||
    ...
    if (t.type === "income") {
      ...
      incomeCategories[t.main_category].subcategories[subCat] =
        (incomeCategories[t.main_category].subcategories[subCat] || 0) + amount;
    } else {
      expense += amount;
      ...
      expenseCategories[t.main_category].subcategories[subCat] =
        (expenseCategories[t.main_category].subcategories[subCat] || 0) +
        amount;
    }
  });
...
function DashboardTotalsTable({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 text-card-foreground rounded-xl border shadow-sm p-4 sm:p-5 mt-4">
      ...
      >
        <h3
          className={`text-base font-semibold ${colorClass} flex items-center gap-2`}
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground">
                <th className="text-left py-2 font-medium w-[30%] sm:w-1/3">
                  Category
                </th>
                ...
                <th className="text-right py-2 font-medium">
                  vs '
                  {new Date().getFullYear().toString().slice(-2) === "00"
                    ? "99"
              ...
              {filteredData.map((item) => (
                <tr key={item.category} className="text-sm">
                  ...
                  <td className="py-3 px-2 text-right">
                    <span
                      className={`${item.isIncome ? "text-green-600" : item.isExpense ? "text-red-600" : ""} text-xs`}
                    >
                      <PrivacyBlur blur={privacyMode}>
    ...
    <div className="bg-white dark:bg-gray-800 text-card-foreground rounded-xl border shadow-sm p-4 sm:p-5 mt-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground">
              <th className="text-left py-2 font-medium w-[25%] sm:w-1/3">
                Category
              </th>
              <th className="text-right py-2 font-medium"></th>
              <th className="text-right py-2 font-medium">Previous</th>
            ...
            {items.map((item) => (
              <React.Fragment key={item.category}>
                ...
                >
                  ...
                  <td className="py-3 px-2 text-right">
                    <span
                      className={`${item.type === "income" ? "text-green-600" : "text-red-600"} text-xs`}
                    >
                      <PrivacyBlur blur={privacyMode}>
                        €{formatAmount(item.total)}
                      </PrivacyBlur>
                ...
                {expandedCategory === item.category &&
                  item.subItems?.map((sub) => (
                    <tr
                      key={`${item.category}-${sub.category}`}
                      className="text-xs bg-gray-50/50 dark:bg-gray-800/50"
                    >
...
export default function DashboardSummary({
  ...
  const totalsData = useMemo(() => {
    if (!current.transactions || !prev.transactions || !lastYear.transactions)
      return null;
        ...
        .forEach(([cat, data]) => {
          ...
          Object.entries(data.subcategories)
            ...
            .forEach(([subCat, subAmount]) => {
              ...
              const lastYearSubTotal =
                lastYearCatData?.subcategories[subCat] || 0;
              subItems.push({
                category: subCat,
                total: subAmount,
    ...
    buildCategoryItems(
      curr.incomeCategories,
      prevT.incomeCategories,
      lastY.incomeCategories,
      "income",
    );
    buildCategoryItems(
      curr.expenseCategories,
      prevT.expenseCategories,
      lastY.expenseCategories,
      "expense",
    );
