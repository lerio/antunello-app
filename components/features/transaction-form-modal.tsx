...
import { parentTransactionId } from "@/utils/transaction-utils";
import {
...
function DeleteSection({
  ...
  // For derived rows (TR bonus transactions, split children), show a link to
  // the originating transaction instead of the delete link — they are
  // unmodifiable and are removed together with their parent.
  if (initialData) {
    const sourceId = parentTransactionId(initialData);
    if (sourceId) {
      return (
        <div className="pt-1 pb-1 space-y-1">
          <button
            type="button"
            onClick={() => onViewOriginal?.(sourceId)}
            className="w-full text-sm text-center hover:underline transition-colors py-1 focus:outline-none"
          >
            View Original Transaction
          </button>
          {initialData.bonus_kind && (
            <p className="text-xs text-center text-gray-400 dark:text-gray-500">
              Generated by Trade Republic import — cannot be edited or deleted
            </p>
          )}
        </div>
      );
